...
def popen_ping(i):
    ...
    for j in range(len(hosts)):
        r_ip = hosts[j]
        key = 'host'+str(i).rjust(2, '0')+'-'+'host'+str(j).rjust(2, '0')
        sql1 = "select rttavg from valu where tss > UNIX_TIMESTAMP()-60 and src='%s' and dst = '%s'" % (s_ip, r_ip)
        cur.execute(sql1)
        avg_time = cur.fetchall()
        print("avg_time",avg_time)
        avg_time1 =0
        if avg_time:
           avg_time1 =avg_time[0][0]
           value = int(float(avg_time1)*100)
           print("value",value)
           mesh_data[key] = [value, "aprilmadaha"]
           print("mesh_data",mesh_data)
    print("mesh_data_meiyou",mesh_data)
    return mesh_data
    # time.sleep(5)
