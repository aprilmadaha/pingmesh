...
&emsp;&emsp;从`表1`中可以得出几个观察结果。 首先，丢包率在0.0001%-0.001%的范围内。 我们每天跟踪所有数据中心的数据包丢弃率，除非发生网络事件，否则我们发现丢弃率在此范围内。 其次，pod之间的数据包丢弃率通常比pod内高几倍。 这表明大多数数据包丢弃都发生在网络而不是主机中。 第三，pod内丢包率约为0.0001%，比我们预期的要大。
